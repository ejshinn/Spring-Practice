...
// @Entity : JPA에서 해당 클래스가 데이터베이스의 테이블이라는 것을 알려주는 애너테이션
// 테이블 생성 시 클래스명이 테이블명으로 사용
// 사용자가 원하는 데이터베이스 테이블명을 사용하고자 할 경우 @Table 애너테이션을 사용함
...
// @NoArgsConstructor : 매개변수가 없는 기본 생성자를 생성하는 애너테이션
@NoArgsConstructor

// @AllArgsConstructor : 모든 필드에 대한 매개변수가 있는 생성자를 자동으로 생성하는 애너테이션
@AllArgsConstructor
@Data
public class JpaBoardEntity {

    // @Id : 해당 필드가 데이터베이스의 기본키임을 나타내는 애너테이션
    @Id
    // @GeneratedValue : 데이터가 추가될 경우 값을 자동으로 증가할지 여부를 지정하는 애너테이션
    ...
    // @Column : 지정한 필드를 데이터베이스의 컬럼과 연동시키는 애너테이션
    // 해당 애너테이션이 없어도 기본적으로 컬럼으로 인식
    ...
    // 데이터 입력 형식 설정, 시간대가 Asia/Seoul로 설정
    @Column(nullable = false)
    ...
    // @Column 애너테이션 미사용 시 기본값으로 모두 적용
