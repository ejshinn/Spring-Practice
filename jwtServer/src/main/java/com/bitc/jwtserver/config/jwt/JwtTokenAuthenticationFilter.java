...
// OncePerRequestFilter : 스프링 시큐리티에서 제공하는 필터로 사용자 요청 시 한 번만 실행하는 필터
@RequiredArgsConstructor
public class JwtTokenAuthenticationFilter extends OncePerRequestFilter {

    private final JwtTokenProvider jwtTokenProvider;

    // JWT 토큰 사용 시 Request 객체의 header에 Authorization라는 키로 토큰 정보가 들어가 있음
    private final static String HEADER_AUTHORIZATION = "Authorization";
    // JWT 토큰 사용 시 첫 번째 단어로 Bearer 가 적혀 있음
    // Bearer 를 제외한 나머지 문자열이 실제 JWT 토큰 문자열임
    // 'Bearer ' 로 총 7자임
    private final static String TOKEN_PREFIX = "Bearer ";

    // OncePerRequestFilter에서 상속받은 메서드, 필터 동작 시 실행되는 메서드
    @Override
    protected void doFilterInternal(
        ...
        if (jwtTokenProvider.validToken(token)) {
            ...
            // setAuthentication() : 인증 정보를 저장하는 메서드
    ...
    private String getAccessToken(String authorizationHeader) {
        // request 객체의 header 정보가 있는지 확인, Bearer 로 시작하는 문자열이 있는지 확인
