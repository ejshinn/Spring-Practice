...
public class JpaBoardServiceImpl implements JpaBoardService {
    ...
    public JpaBoardEntity selectBoardDetail(int boardIdx) throws Exception {
        // Optional : 데이터베이스에서 데이터를 조회 시 조회된 데이터가 null일 경우 이 null은 자바의 null과 의미가 조금 다름
        // 데이터베이스의 null은 특정 컬럼에 데이터가 없다는 의미로 사용되고, 자바의 null은 객체의 주소가 없다는 의미
    ...
    public void saveBoard(JpaBoardEntity board) throws Exception {
        // JpaRepository에서 제공하는 save() 메서드를 사용하여 insert와 update 기능을 구현함
        jpaBoardRepository.save(board);
    }
    ...
    public void deleteBoard(int boardIdx) throws Exception {
        // JpaRepository에서 제공하는 deleteById() 메서드를 사용하여 delete 기능을 구현함
