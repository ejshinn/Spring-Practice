...
public class AjaxController {
    ...
    // @RequestParam : 클라이언트에서 전달한 데이터를 가져오는 애너테이션, jsp의 request.getParameter()와 같은 기능
    // 매개변수를 추가하여 여러 가지 옵션을 사용할 수 있음
    // value : 클라이언트에서 전달한 데이터의 파라미터명을 지정
    // required : true(기본값)/false로 설정, 해당 파라미터의 값이 없을 경우 오류를 표시(false 사용 시 오류 미표시)
    // defaultValue : required 속성과 함께 사용하여 사용자가 해당 파라미터에 값을 입력하지 않을 경우 defaultValue로 설정된 값을 사용하게 함

    // @RequestMapping : 해당 메서드와 사용자가 접속할 URL을 매칭하는 애너테이션
    // value : 사용자가 접속할 URL 설정
    // method : 사용자가 서버로 데이터 전달 시 사용한 통신 방식을 설정(GET, POST), Restful 방식 사용 시 GET, POST, PUT, DELETE를 사용할 수 있음

    // @ResponseBody : 응답으로 리턴할 데이터를 html 템플릿이 아닌 데이터 자체를 전달하도록 하는 애너테이션, ajax 통신 시 반드시 필요함
    @ResponseBody
