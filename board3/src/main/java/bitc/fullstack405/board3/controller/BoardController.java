...
public class BoardController {
    ...
    // HTTP URI를 통해서 자원을 명시하고, HTTP Method(GET, POST, PUT, DELETE)를 통해 자원에 대한 CRUD 명령을 적용하는 방식

    // URI : Uniform Resource Identifier의 줄임말, 자원 식별자
    // URL : Uniform Resource Locator의 줄임말, 자원의 위치
    // 현재는 거의 같은 의미로 사용됨
    ...
    // @RequestMapping 애너테이션 사용 시 URI를 value 속성을 사용하여 설정, method 속성을 추가하여 GET, POST, PUT, DELETE를 사용하여 설정
    // @GetMapping, @PostMapping, @PutMapping, @DeleteMapping 애너테이션을 사용하여 구분해도 상관없음

    // Spring Framework의 기본 설정은 GET, POST 방식만 인식함
    // 설정 파일인 application.properties에 spring.mvc.hiddenmethod.filter.enabled=true가 추가되야 함

    // Spring Framework의 템플릿에서 GET, POST 방식만 인식하기 때문에 PUT, DELETE를 인식하기 위해서 input type=hidden을 추가하고, name=_method 속성을 사용하여 데이터 전송 방식(PUT, DELETE)을 지정해야 함
    ...
    // @GetMapping : 클라이언트에서 데이터 전송 방식을 GET으로 설정한 URL만 접속, @RequestMapping(method = RequestMethod.GET)과 동일한 방식
    // 게시물 등록(view)
    ...
    // @PostMapping : 클라이언트에서 데이터 전송 방식을 POST로 설정한 URL만 접속, @RequestMapping(method = RequestMethod.POST)와 동일한 방식
    // 게시물 등록(내부 처리)
    ...
    // @PutMapping : 클라이언트에서 데이터 전송 방식을 PUT으로 설정한 URL만 접속, @RequestMapping(method = RequestMethod.PUT)와 동일한 방식
    // 게시물 수정
    ...
    // @DeleteMapping : 클라이언트에서 데이터 전송 방식을 DELETE로 설정한 URL만 접속, @RequestMapping(method = RequestMethod.DELETE)와 동일한 방식
    // @PathVariable : @RequestParam과 동일한 역할을 하는 애너테이션, REST 방식 사용 시 URI에 {}로 지정해 놓은 리소스 값을 받아오는 애너테이션
    // 게시물 삭제
