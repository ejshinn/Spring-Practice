...
// @Controller : 해당 클래스가 Spring WEB MVC의 Controller 파일임을 스프링 프레임워크에 알려주는 애너테이션
// JSP MVC2 방식의 Servlet 파일과 동일한 역할을 함
// JSP Servlet에서는 URL 하나에 Servlet 파일 하나를 연동하여 사용하였지만 Spring MVC의 Controller는 하나의 Controller 파일에 @RequestMapping을 사용하여 메서드에 URL을 연동하여 사용
// 하나의 Controller 파일에 여러 개의 URL을 사용할 수 있음

// @RestController : @Controller 애너테이션과 동일한 역할을 하는 애너테이션, 클라이언트로 전달하는 것이 view 파일이 아닌 데이터를 직접 전달할 경우 사용하는 애너테이션
@Controller
public class BoardController {
    ...
    // @RequestMapping : 클라이언트에서 접속할 URL을 지정하는 애너테이션, JSP의 @WebServlet과 비슷한 기능을 함, @RequestMapping은 클래스와 메서드 모두 사용할 수 있음
    // 메서드에 사용 시 실제 클라이언트가 접속할 URL로 사용
    // 클래스에 사용 시 기본 URL로 사용됨(클래스에 지정한 URL + 메서드에 지정한 URL이 최종 URL)
    // 여러 개의 URL을 처리하고자 할 경우 {"주소1", "주소2", ...} 형식으로 사용할 수 있음
    @RequestMapping({"/", "/home", "/index"})
    public String index() {
        // 반환값으로 사용되는 String 데이터는 실제 클라이언트에 출력될 템플릿 파일명을 지정함
    ...
    public ModelAndView boardList() throws Exception {
        // ModelAndView : 클라이언트에서 화면에 출력되는 view와 데이터가 함께 포함된 클래스
    ...
    // DTO 타입으로 클라이언트에서 데이터를 받아올 경우 클라이언트의 input 태그 name 속성값이 DTO 클래스의 필드명과 동일해야 함

    // 첨부파일 정보를 입력받기 위해서 MultipartHttpServletRequest 클래스 타입의 객체를 매개변수로 받음
    ...
    public String insertBoard(BoardDTO board, MultipartHttpServletRequest multipart) throws Exception {
        // 서비스 객체에서 제공하는 insertBoard() 메서드를 호출
        // boardService.insertBoard(board);
        // 서비스 객체에서 제공하는 insertBoard() 메서드와 파일 정보를 함께 저장
    ...
    // 글 상세보기
    // @RequestParam : 클라이언트에서 전달한 데이터의 변수명을 알려주는 애너테이션, 데이터 전달 기능
