...
// @RestController : @Controller + @ResponseBody 애너테이션이 하나로 합쳐진 애너테이션
// @Controller의 기능인 스프링 프레임워크에 해당 클래스가 Controller임을 알려주고 클라이언트에서 접속하는 URL과 메서드를 연결하는 기능과 @ResponseBody의 기능인 클라이언트에게 View 없이 데이터만 전달하는 기능이 합쳐진 애너테이션임
// @Controller 애너테이션과 @ResponseBody 애너테이션을 따로 입력하여 사용하여도 같은 효과를 가짐
...
// 예전에는 클라이언트가 컴퓨터밖에 없었기 때문에 웹 브라우저 혹은 전용 프로그램이 클라이언트의 기능을 모두 담당했음
// 서버 기능의 웹 서버 기능만 주로 사용되었기 때문에 클라이언트로 전달하는 데이터가 View와 Model 데이터 모두 필요한 형태였음
