...
// @Service : 스프링 프레임워크에 해당 파일이 서비스 파일임을 알려주는 애너테이션
@Service
// BoardService를 상속받아 구현하는 클래스
public class BoardServiceImpl implements BoardService {

    // 데이터베이스를 사용하기 위한 Mapper 인터페이스 타입의 변수 선언
    // @Autowired 애너테이션을 사용하여 스프링 프레임워크가 자동으로 객체를 생성하고 관리함
    @Autowired
    ...
    // BoardService 인터페이스에서 상속받아 구현한 메서드
    @Override
    public List<BoardDTO> selectBoardList() throws Exception {
        // BoardMapper에서 제공하는 selectBoardList() 메서드를 사용하여 데이터를 가져옴
        return boardMapper.selectBoardList();
    }

//    @Override
//    public void insertBoard(BoardDTO board) throws Exception {
//        // 데이터베이스에 데이터를 추가하기위해 mapper에서 제공하는 insertBoard() 메서드 호출
...
//    }
    ...
    public void insertBoard(BoardDTO board, MultipartHttpServletRequest multipart) throws Exception {
        // mapper를 사용하여 DB에 게시물 등록
        boardMapper.insertBoard(board);

        // 업로드된 파일 정보를 모두 가져옴, 해당 정보를 가지고 DB의 t_files 테이블에 데이터를 저장할 수 있음
        // FileUtils 클래스 타입의 객체에서 제공하는 parseFileInfo()를 호출하여 BoardFileDTO 클래스 타입의 객체가 저장된 List를 가져옴
