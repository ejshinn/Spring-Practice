...
public interface ProductRepository extends JpaRepository<ProductEntity, Long> {
    ...
    // JpaRepository를 상속받았기 때문에 JpaRepository가 제공하는 기본 메서드와 쿼리 메서드를 생성하여 사용할 수 있음

    // 쿼리 메서드 : JpaRepository에서 제공하는 기본 메서드로는 원하는 데이터를 모두 컨트롤 할 수 없기 때문에 JpaRepository에서 제공하는 주제 키워드를 조합하여 원하는 메서드를 생성할 수 있음

    // 쿼리 메서드 생성 방법
    // 리턴타입 + {주제 키워드 + 서술어}를 조합하여 메서드를 생성 ({} 부분은 반복 사용 가능)
    ...
    // delete ... By, remove ... By : delete 명령을 수행하는 키워드, delete ... By는 리턴이 없음, remove ... By는 삭제 수를 반환함
    void deleteByNumber(Long number); // 지정한 번호를 기준으로 데이터 삭제, 반환 값 없음
    ...
    // Is : 조건 키워드, 값의 일치 여부를 조건으로 사용, Equals와 동일한 기능 제공
    ...
    List<ProductEntity> findByPriceIsBetween(long lowPrice, long highPrice); // 지정한 낮은 데이터부터 높은 데이터까지의 모든 데이터 조회
    ...
    List<ProductEntity> findByNameOrderByNumberAsc(String name); // 지정한 이름으로 데이터를 조회한 후 번호를 기준으로 오름차순 정렬
    List<ProductEntity> findByNameOrderByNumberDesc(String name); // 지정한 이름으로 데이터를 조회한 후 번호를 기준으로 내림차순 정렬
    List<ProductEntity> findByNameOrderByPriceAscStockDesc(String name); // 지정한 이름으로 데이터를 조회한 후 가격을 기준으로 오름차순 정렬을 먼저 진행하고, 재고 수량을 기준으로 내림차순 정렬

    // 정렬 사용 시 Sort 객체를 사용하여 정렬하는 것이 가능함
    // 쿼리 메서드 생성 후 매개변수에 Sort 객체를 추가하여 정렬
    // List<ProductEntity> findByName(String name);와 같이 쿼리 메서드를 생성 후 실행 시 Sort 객체를 추가하여 실행
    ...
    // @Query : 복잡한 쿼리문은 쿼리 메서드로 생성하기 힘들기 때문에 @Query라는 애너테이션을 제공함
    ...
    // SELECT절에 클래스의 별명.컬럼명으로 출력하고자 하는 컬럼만 입력
    ...
    // 메서드의 매개변수를 쿼리문에 적용할 경우 '?순번' 형식으로 입력하여 데이터를 매칭함
    ...
    // 메서드의 매개변수를 쿼리문에 적용할 경우 ':매개변수명' 형식으로 입력하여 데이터를 매칭함
    // ':매개변수명' 방식은 메서드의 매개변수 앞에 @Param("컬럼명") 애너테이션을 사용해야 함
